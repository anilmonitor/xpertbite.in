...
export function DurgaPujaClient({ initialGreeting }: { initialGreeting?: any }) {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          try {
            localStorage.setItem("durga_puja_user_photo", compressed);
          } catch {}
  ...
  // Robust universal WhatsApp opener (Direct app trigger on mobile + web fallback)
  const triggerWhatsApp = (messageText: string) => {
    ...
    if (isMobile) {
      // 1. Native WhatsApp App Direct Intent (Opens directly without browser prompt)
      window.location.href = `whatsapp://send?text=${encoded}`;

      // 2. Safety Fallback if native scheme doesn't open within 1.2s
      setTimeout(() => {
  ...
  // Fast 100% Reliable 1-Tap WhatsApp Share
  ...
  return (
    <div className="relative min-h-screen bg-[#FFFDF9] text-gray-900 overflow-x-hidden font-hindi-heading flex flex-col items-center justify-start pt-16 sm:pt-20 pb-12 px-4 sm:px-6">
      ...
      <div className="relative z-10 w-full max-w-lg sm:max-w-xl text-center text-gray-900 my-auto space-y-6">
        ...
        {/* Main Divine / User Portrait (Complete Full Photo with Animated Durga Puja GIFs) */}
        <div className="relative w-full max-w-[280px] sm:max-w-xs md:max-w-sm mx-auto rounded-3xl p-2 bg-gradient-to-tr from-amber-400 via-amber-300 to-amber-500 shadow-xl">
          ...
          {/* Animated Durga Puja GIFs at bottom corners of the photo frame */}
          <div className="absolute -bottom-4 -left-4 w-12 h-12 sm:w-14 sm:h-14 z-20 drop-shadow-md pointer-events-none">
            <Image src="/durgapuja/durgapuja.gif" alt="Durga Puja GIF" fill className="object-contain" unoptimized />
          </div>
        ...
        <div className="flex items-center justify-center gap-2">
          <div className="relative w-6 h-6 sm:w-8 sm:h-8 shrink-0">
            <Image src="/durgapuja/durgapuja.gif" alt="Durga GIF" fill className="object-contain" unoptimized />
          </div>
